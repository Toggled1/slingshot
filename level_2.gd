...
func _ready():
	Global.level = ("res://level_2.tscn")
	
func _process(delta):
	$Camera2D.position.y = $hello.position.y - 150
	$Camera2D.position.x = 576 + ($hello.position.x - 576) / 4
	$Camera2D.limit_bottom = 800 + x
	x -= 1.5
	if $hello.position.y - x > 830:
